...
from data_and_models import jansen_rit_strang


class model_fn:
    def __new__(cls, config):
        jax_data_fn, _, jax_simulator_fn, *_ = jansen_rit_strang.model_fn(config)
        ...
        def prior_fns():
            p = torch.distributions.Independent(
                torch.distributions.Uniform(
                    low=torch.as_tensor([10.0, 50.0, 100.0, -20.0], dtype=torch.float32),
                    high=torch.as_tensor([250.0, 500.0, 5000.0, 20.0], dtype=torch.float32),
                ),
                1,
            )
            return p
