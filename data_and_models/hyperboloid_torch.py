...
from data_and_models import hyperboloid


class model_fn:
    def __new__(cls, config):
        jax_data_fn, _, jax_simulator_fn, *_ = hyperboloid.model_fn(config)
        ...
        def prior_fns():
            p = torch.distributions.Independent(
                torch.distributions.Uniform(
                    low=torch.as_tensor(-2.0 * torch.ones(config.ndim_theta), dtype=torch.float32),
                    high=torch.as_tensor(2.0 * torch.ones(config.ndim_theta), dtype=torch.float32),
                ),
                1,
            )
            return p
