...
def as_batch_iterators(rng_key: chex.PRNGKey, data: named_dataset, batch_size, split, shuffle):
    n = data.y.shape[0]
    n_train = int(n * split)
...
# pylint: disable=missing-function-docstring
def as_batch_iterator(rng_key: chex.PRNGKey, data: named_dataset, batch_size, shuffle):
    n = data.y.shape[0]
    if n < batch_size:
        num_batches = 1
        batch_size = n
